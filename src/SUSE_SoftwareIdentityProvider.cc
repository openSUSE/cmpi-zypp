...
CmpiStatus SUSE_SoftwareIdentityProviderClass::getInstance( const CmpiContext &ctx, CmpiResult &rslt, const CmpiObjectPath &cop, const char **properties )
{
  ...
  if ( ! pi )
  {
    ...
    // found one...
  rslt.returnData( SUSE_SoftwareIdentityProviderClass::makeSoftwareIdentityInstance( pi, *zyppac, cop, properties ) );
...
CmpiInstance SUSE_SoftwareIdentityProviderClass::makeSoftwareIdentityInstance( const PoolItem & pi, ZyppAC & zyppac, const CmpiObjectPath & cop, const char** properties )
{
...
CmpiObjectPath SUSE_SoftwareIdentityProviderClass::makeSoftwareIdentityOP( const PoolItem & pi, ZyppAC & zyppac, const CmpiObjectPath & cop)
{
  CmpiObjectPath op( cop.getNameSpace(), _ClassName );
  op.setKey( "InstanceID", zyppac.SoftwareIdentityInstanceId( pi ).c_str() );
  return op ;
}
